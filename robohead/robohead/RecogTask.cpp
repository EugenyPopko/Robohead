...
void RecogTask::PCA(void)
{
	...
	//ћетод главных компонент (Principal Component Analysis, PCA) 
	//примен€етс€ дл€ сжати€ информации без существенных потерь информативности.

	//nTrainFaces - число эталонов, 
	//faceImgArr - указатель на массив изображений-эталонов,
	//eigenVectArr Ц (выход функции) указатель на массив собственных объектов (изображени€ глубиной 32 бит)
	//CV_EIGOBJ_NO_CALLBACK Ц флаги ввода/вывода. ƒл€ работы с пам€тью.
	//0-размер буфера. ƒл€ работы с пам€тью.
	//0-указатель на структуру дл€ работы с пам€тью.
	//calcLimit-критерий прекращени€ вычислений. ƒва варианта: по количеству итераций и по ко точности
...
int RecogTask::recognize(void)
{
    ...
    //функци€ вычислени€ компонентов разложени€
...
void RecogTask::training(void)
{
    ...
    storeTrainingData(); //сохран€ем данные
...
{
    int i,j;
	CvSeq *faces = cvHaarDetectObjects(img, cascade_f, storage, 1.1, 3, CV_HAAR_FIND_BIGGEST_OBJECT | CV_HAAR_DO_ROUGH_SEARCH, cvSize(130, 130)); //классификатор (каскад ’аара)
	...
	cvResize(img1, sizedImg, CV_INTER_LINEAR); //подгон€ем лицо под размер
